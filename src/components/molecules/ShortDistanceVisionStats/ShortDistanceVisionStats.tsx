...
import { getAverageTestScore, getAverageTestScoreNear } from '../../../api/tests';
...
const ShortDistanceVisionStats = ({ user }: { user: UserType }) => {
  ...
  const getAverageTestStats = async () => {
    if (
      ...
      const { apiError, apiSuccess } = await getAverageTestScoreNear(
        user?.data?.otherDetails?._id ?? userId
      );
      if (apiSuccess) {
  ...
  return (
    <View style={{
      ...
      >
        Overview of the near vision tests
      </Text>
      <View>
        <View
          style={{
...
export default ShortDistanceVisionStats;
